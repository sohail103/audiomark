...
extern muriscv_nn_context ctx;

ee_status_t
s_riscv_nn_classify(const input_tensor_t in_data, output_tensor_t out_data)
{
    int8_t *in_out_buf_0
    ...
    status |= muriscv_nn_avgpool_s8(&ctx,
                                    &pool_params,
                                    &in_out_dim_1,
                                    in_out_buf_0,
                                    &conv_filter_dims,
    ...
    return status == MURISCV_NN_SUCCESS ? EE_STATUS_OK : EE_STATUS_ERROR;
}
