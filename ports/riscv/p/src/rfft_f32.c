            ...
            uint8_t                       ifftFlag)
{
    ...
    if (ifftFlag == 1U)
    {
        ...
        riscv_bitreversal_32_inpl(
            (uint32_t *)q31_out, S_CFFT->bitRevLength, S_CFFT->pBitRevTable);
    ...
    else
    {
        ...
        riscv_bitreversal_32_inpl(
            (uint32_t *)q31_in, S_CFFT->bitRevLength, S_CFFT->pBitRevTable);
                     ...
                     uint32_t     fftLen) /* half-length = N/2 */
{
    ...
    while (k > 0)
    {
        ...
        q31x2_t xAB = __riscv_pasa_x_i32x2(
            cmplxA, __riscv_ppairoe_i32x2(cmplxB, cmplxB));
