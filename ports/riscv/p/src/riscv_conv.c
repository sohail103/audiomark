...
void riscv_q31_to_float(const q31_t *pSrc, float *pDst, uint32_t blockSize)
{
    ...
    while (blkCnt > 0U) {
        pDst[i] = (float)pSrc[i] * inv_multiplier; i++;
        pDst[i] = (float)pSrc[i] * inv_multiplier; i++;
        pDst[i] = (float)pSrc[i] * inv_multiplier; i++;
        pDst[i] = (float)pSrc[i] * inv_multiplier; i++;
        blkCnt--;
...
void riscv_q31_to_float_unnormalize(const q31_t *pSrc, float *pDst, uint32_t blockSize, float inv_multiplier)
{
    uint32_t blkCnt = blockSize >> 2U;
    uint32_t i = 0;
...
void riscv_float_to_q31(const float *pSrc, q31_t *pDst, uint32_t blockSize)
{
    ...
    while (blkCnt > 0U) {
        in = pSrc[i] * 2147483648.0f; in += (in > 0.0f) ? 0.5f : -0.5f; pDst[i++] = (q31_t)in;
        in = pSrc[i] * 2147483648.0f; in += (in > 0.0f) ? 0.5f : -0.5f; pDst[i++] = (q31_t)in;
        in = pSrc[i] * 2147483648.0f; in += (in > 0.0f) ? 0.5f : -0.5f; pDst[i++] = (q31_t)in;
        in = pSrc[i] * 2147483648.0f; in += (in > 0.0f) ? 0.5f : -0.5f; pDst[i++] = (q31_t)in;
        blkCnt--;
...
float riscv_float_to_q31_normalize(const float *pSrc, q31_t *pDst, uint32_t blockSize)
{
    ...
    while (blkCnt > 0U) {
        in = pSrc[i] * combined_multiplier; in += (in > 0.0f) ? 0.5f : -0.5f; pDst[i++] = (q31_t)in;
        in = pSrc[i] * combined_multiplier; in += (in > 0.0f) ? 0.5f : -0.5f; pDst[i++] = (q31_t)in;
        in = pSrc[i] * combined_multiplier; in += (in > 0.0f) ? 0.5f : -0.5f; pDst[i++] = (q31_t)in;
        in = pSrc[i] * combined_multiplier; in += (in > 0.0f) ? 0.5f : -0.5f; pDst[i++] = (q31_t)in;
        blkCnt--;
